...
const callArgs = new Argument('[args]', 'JSON-encoded arguments array of contract call')
  .argParser((argsText) => {
    ...
    if (!Array.isArray(args))
      throw new CliError(`Call arguments should be an array, got ${argsText} instead`);
    return args;
  })
  .default([]);

const descriptorPathOption = new Option(
  '-d, --descrPath [descrPath]',
  'Path to contract descriptor file',
);
// TODO: remove "contract" prefix because it is clear in context
const contractSourceFilenameOption = new Option(
  '--contractSource [contractSource]',
  'Contract source code file name',
);
const contractAciFilenameOption = new Option(
...
command = program
  ...
  .addOption(
    new Option('-t, --topHash', 'Hash of block to make call').implies({
      callStatic: true,
    }),
  )
