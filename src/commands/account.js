...
let command = program.command('sign <wallet_path> <tx>')
...
command = program.command('sign-message <wallet_path> [data...]')
  .option('--filePath [path]', 'Specify the path to the file for signing (ignore "data" argument and use file instead)')
...
command = program.command('verify-message <address> <hexSignature> [data...]')
  .option('--filePath [path]', 'Specify the path to the file (ignore "data" argument and use file instead)')
  .summary('check if message was signed by address')
  .action(Account.verifyMessage);
addCommonOptions(command, [`${exampleAddress1} f2f268f195d4747568f38f9efd36e72606dc356c0b8db9fdfae5f1f9c207dbc354c57c29397837d911516aef184b0ddbed7d16d77caf9ffb3f42fe2bcc15c30e ${exampleMessage}`]);

command = program.command('address <wallet_path>')
...
command = program.command('create <wallet_path>')
